...
from mediafiles.utils import check_settings, matches_patterns


class MediaFilesStorage(FileSystemStorage):
    """
    Standard file system storage for media files.
    ...
    def __init__(self, location=None, base_url=None, *args, **kwargs):
        ...
        super(MediaFilesStorage, self).__init__(location, base_url,
                                                 *args, **kwargs)

    def path(self, name):
        if not self.location:
            raise ImproperlyConfigured("You're using the mediafiles app "
                                       "without having set the MEDIA_ROOT "
                                       "setting to a filesystem path.")
        return super(MediaFilesStorage, self).path(name)
...
class CachedMediaFilesStorage(CachedFilesMixin, MediaFilesStorage):
    """
    A media file system storage backend which also saves
...
class AppMediaStorage(FileSystemStorage):
    ...
    def __init__(self, app, *args, **kwargs):
        """
        Returns a media file storage if available in the given app.
        """
        # app is the actual app module
        mod = import_module(app)
        mod_path = os.path.dirname(upath(mod.__file__))
        location = os.path.join(mod_path, self.source_dir)
        super(AppMediaStorage, self).__init__(location, *args, **kwargs)


class ConfiguredStorage(LazyObject):
    def _setup(self):
        self._wrapped = get_storage_class(settings.MEDIAFILES_STORAGE)()

mediafiles_storage = ConfiguredStorage()
